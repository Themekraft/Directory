...
function tk_ud_posts_where( $where, $tk_ud_search_query = '' ) {
    ...
    $querystr = $tk_ud_search_query->get('s');

    //echo $where;
	if( $queryid == 'tk_ud_search' ){
		// $where = str_replace( 'AND (((wp_posts.post_title LIKE \'%', 'AND (((wp_posts.post_title LIKE \'', $where );
		$where = str_replace( "AND (((wp_posts.post_title LIKE '%$querystr%') OR (wp_posts.post_excerpt LIKE '%$querystr%') OR (wp_posts.post_content LIKE '%$querystr%'))) ", "AND wp_posts.post_title LIKE '$querystr%' ", $where );
